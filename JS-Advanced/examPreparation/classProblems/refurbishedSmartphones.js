class RefurbishedSmartphones {
    constructor(retailer) {
        this.retailer = retailer;
        this.availableSmartphones = [];
        this.soldSmartphones = [];
        this.revenue = 0;
    }

    addSmartphone(model, storage, price, condition) {
        if (model === "" ||
            storage < 0 ||
            !Number.isInteger(storage) ||
            price < 0 ||
            condition === "") {
            throw new Error('Invalid smartphone!');
        };

        this.availableSmartphones.push({ model, storage, price, condition });
        return `New smartphone added: ${model} / ${storage} GB / ${condition} condition - ${price.toFixed(2)}$`
    }
    sellSmartphone(model, desiredStorage) {
        let findPhone = this.availableSmartphones.find(x => x.model === model);
        if (!findPhone) {
            throw new Error(`${model} was not found!`)
        };

        if (findPhone.storage >= desiredStorage) {
            findPhone.price = findPhone.price;
        } else if (desiredStorage - findPhone.storage <= 128) {
            findPhone.price -= findPhone.price * 0.1;
        } else if (desiredStorage - findPhone.storage > 128) {
            findPhone.price -= findPhone.price * 0.2;
        };

        let idx = this.availableSmartphones.indexOf(findPhone);
        this.availableSmartphones.splice(idx, 1);
        this.soldSmartphones.push({ model: findPhone.model, storage: findPhone.storage, soldPrice: findPhone.price });
        this.revenue += findPhone.price;
        return `${model} was sold for ${findPhone.price.toFixed(2)}$`

    }

    upgradePhones() {
        if (this.availableSmartphones.length == 0) {
            return `There are no available smartphones!`;
        }

        for (let phone of this.availableSmartphones) {
            phone.storage *= 2;
        }

        return (`Upgraded Smartphones:` +
            "\n" +
            this.availableSmartphones.map(x => `${x.model} / ${x.storage} GB / ${x.condition} condition / ${x.price.toFixed(2)}$`).join('\n')
        )
    }

    salesJournal(criteria) {

        if (this.soldSmartphones.length == 0) {
            throw new Error('There are no available smartphones!')
        }
        if (criteria !== 'storage' &&
            criteria !== 'model') {
            throw new Error('Invalid criteria!')
        };

        let sortedArr = [];
        if (criteria == 'storage') {
            sortedArr = this.soldSmartphones.sort((a, b) => b.storage - a.storage);
        } else if (criteria == 'model') {
            sortedArr = this.soldSmartphones.sort((a, b) => a.model.localeCompare(b.model));
        };

        return (
            `${this.retailer} has a total income of ${this.revenue.toFixed(2)}$` +
            "\n" +
            `${this.soldSmartphones.length} smartphones sold:` +
            "\n" +
            sortedArr.map(x => `${x.model} / ${x.storage} GB / ${x.soldPrice.toFixed(2)}$`).join('\n')
        );

    };
}
let retailer = new RefurbishedSmartphones('SecondLife Devices');
retailer.addSmartphone('Samsung S20 Ultra', 0, 1000, 'good');
retailer.addSmartphone('Iphone 12 mini', 128, 800, 'perfect');
retailer.addSmartphone('Aiaomi Redmi Note 10 Pro', 512, 330, 'perfect');
retailer.sellSmartphone('Samsung S20 Ultra', 256);
retailer.sellSmartphone('Aiaomi Redmi Note 10 Pro', 256);
console.log(retailer.salesJournal('model'));








